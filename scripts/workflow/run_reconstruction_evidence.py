#!/usr/bin/env python3
"""Capture focused FCEUX evidence for resolved reconstruction findings."""
...
def main() -> int:
    ...
    for scenario in document["scenarios"]:
        ...
        environment.update(
            PACMAN_RECONSTRUCTION_EVIDENCE_TRACE=str(output.resolve()).replace("\\", "/"),
            PACMAN_RECONSTRUCTION_EVIDENCE_SCENARIO=scenario["id"],
        ...
        if not output.is_file() or output.stat().st_size == 0:
            raise SystemExit(f"[FAIL] Reconstruction evidence was not created: {output}")
        print(f"[OK] Reconstruction evidence: {output}")
    return 0
