...
sub_draw_score_hud_live:		; was: sub_E379
...
sub_draw_score_hud_dual:		; was: sub_E393
    PrepareScoreHud $2083, $09
    LDA #$01
    STA ram_game_mode
    LDX #$0C
; Select score-address triplet based on mode/player
bra_select_score_addr_triplet:		; was: bra_E3A5
    LDA ram_game_mode
    AND ram_current_player
...
bra_init_hud_row_counter:		; was: bra_E3B0
    LDA #$03
    STA ram_score_hud_row_count
; Draw next HUD row packet
bra_draw_next_hud_row:		; was: bra_E3B4_loop
...
bra_continue_hud_row_loop:		; was: bra_E3E9
    LDA ram_score_hud_row_count
    BNE bra_draw_next_hud_row
; Return from HUD row draw routine
bra_return_from_score_hud_draw:		; was: bra_E3ED_RTS
    RTS

; Write one 6-digit score value to PPU with leading-space suppression
sub_write_6digit_score_to_ppu:		; was: sub_E3EE
    LDY #$05
; Skip leading zeros while writing score digits
bra_skip_leading_zeros_in_score:		; was: bra_E3F0_loop
...
bra_write_remaining_score_digits:		; was: bra_E400_loop
...
bra_write_last_digit_and_suffix:		; was: bra_E40B
; Y = 00
...
tbl_score_source_triplets:		; was: tbl_E419_score_addr
; 00
    .word ram_score_hi
    .word ram_score_p1
    .word ram_score_p2
