...
bra_init_anim_phase_offset:
    LDA #$01
    STA zp_work1
; Loop over ghost slots for anim update
bra_update_ghost_anim_slot_loop:
...
handler_state07_anim_noop:
    RTS

; Dispatch ghost animation handler by state
sub_dispatch_ghost_anim_handler:
...
bra_build_anim_frame_from_direction:
    LDA ram_ghost_direction,X
    STA zp_work3
    LDA #$0A
; Accumulate frame offset by direction
bra_accumulate_anim_frame_offset:
    DEC zp_work3
...
bra_apply_anim_phase_offset:
    CLC
    ADC zp_work0
    STA zp_work2
; Store ghost anim frame for current slot
bra_store_ghost_anim_frame:
    TXA
    LSR
    TAY
    LDA zp_work2
...
handler_state00_anim_default:
; State02 anim handler
handler_state02_anim_default:
; State04 anim handler
handler_state04_anim_default:
    LDA zp_work1
...
handler_state06_anim_eyes:
    LDA ram_ghost_direction,X
    STA zp_work3
    LDA #$20
; Accumulate eyes animation offset
bra_accumulate_eyes_anim_offset:
    DEC zp_work3
...
sub_prepare_sprite_positions:
    LDX #$23
; Copy object positions to sprite position buffer
bra_copy_obj_to_sprite_pos:
...
bra_finalize_positions:
; Finalize positions and continue to OAM compose
loc_finalize_positions:
