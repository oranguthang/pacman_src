...
## Entry Points
- `sub_check_for_eating_pellets`: pellet/power-pellet detection at Pac-Man tile.
- `loc_add_points_and_update_score_buffers`: central score commit/format/hiscore path.
...
- Power-pellet branch `DEF7`:
  - enters frightened mode (`sub_start_frightened_mode`)
...
- Loads pending score delta from `tbl_pellet_clear_tile_and_points` into `ram_pending_score_bcd`.
...
- Spawn thresholds in remaining pellet count:
  - `0x86` and `0x37` -> `bra_spawn_fruit_item`.
- Spawn writes:
  - timer (`ram_fruit_timer_hi/lo = 0A:3C`)
  - sprite tile by stage via `tbl_fruit_sprite_tile_by_stage`
...
5. 1UP check (`ram_extra_life_awarded` latch) and life increment + life-icon packet append.
6. Compare score vs hiscore digit-by-digit and promote both PPU+RAM buffers if exceeded.

## Related Tables
- `tbl_pellet_clear_tile_and_points`: pellet clear tile + point deltas.
- `tbl_fruit_sprite_tile_by_stage`: fruit sprite tile per stage group.
- `tbl_life_icon_ppu_packets`: packet template for life icon update.

## RAM Contracts
- `ram_pending_score_bcd`: six pending BCD score-delta digits written by event producers.
- `ram_score_p1` / `ram_score_hi`: canonical score/hiscore digits.
- `ram_ppu_buffer_score` / `ram_ppu_buffer_hiscore`: display buffers.
- `ram_extra_life_awarded`: one-time 1UP awarded latch.
