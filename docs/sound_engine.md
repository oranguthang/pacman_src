...
not necessarily independent audible effects. Slots `$0608..$060E` are internal
house, release-counter, and intermission markers confirmed by the focused
reconstruction evidence trace.
...
caller-based labels whose same-index stream activation is confirmed by resolved
entry `SND-001`.
...
- Keep opcode table entries and dormant handlers until stronger evidence permits
  removal.
- Run `make trace-evidence` after changing request arbitration; it requires all
  16 same-index request/stream slots to activate across gameplay and pause.
